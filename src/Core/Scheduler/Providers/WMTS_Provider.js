...
function getColorTexture(coordWMTS, layer, targetZoom) {
    ...
    if (targetZoom && targetZoom !== coordWMTS.zoom) {
        const c = coordWMTS;
        coordWMTS = OGCWebServiceHelper.WMTS_WGS84Parent(coordWMTS, targetZoom);
        c.offsetToParent(coordWMTS, pitch);
    }
...
function getColorTextures(tile, layer, targetLevel) {
    ...
    for (const coordWMTS of bcoord) {
        promises.push(getColorTexture(coordWMTS, layer, targetLevel));
    }
